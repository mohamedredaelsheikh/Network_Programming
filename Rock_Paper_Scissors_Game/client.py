...
def redrawWindow(window, game, player):
    ...
    if not(game.connected()):
        # Display waiting message if not connected
        ...
        if game.bothWent():
            ...
            # Display the players state either "locked in or waiting "
            if game.player_1_Went and player == 0:
                text1 = font.render(move1, 1, (0,0,0))
            elif game.player_1_Went:
                text1 = font.render("Locked In", 1, (0, 0, 0))
            else:
                text1 = font.render("Waiting...", 1, (0, 0, 0))

            if game.player_2_Went and player == 1:
                text2 = font.render(move2, 1, (0,0,0))
            elif game.player_2_Went:
...
def main():
    ...
    while run:
        ...
        if game.bothWent():
            # Display game result and reset game
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for btn in btns:
                    if btn.click(pos) and game.connected():
                        ...
                        if player == 0:
                            if not game.player_1_Went:
                                n.send(btn.text)
                        else:
                            if not game.player_2_Went:
                                n.send(btn.text)
